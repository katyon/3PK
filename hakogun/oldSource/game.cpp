...
void	Game::Initialize(ID3D11Device* device)
{
    ...
    //	「地面」を初期化
    field.Initialize();
    field.SetPrimitive(new GeometricRect(device));
    ...
    //　ポータルを設置
    portal.Initialize();
    portal.SetPrimitive(new GeometricSphere(device));
    ...
    pWaveManager->init(device, modelNames01, waveData01);
...
bool	Game::Update(ID3D11Device* device)
{

    player.Move(device);					//	「プレイヤー」の移動処理
    enemyManager.Update(device);			//	「敵管理」の更新処理
    camera.Update();
...
void	Game::Render(ID3D11DeviceContext* context)
{
    ...
    field.Render(context, view, projection, light_direction);			//	「地面」の描画処理
    portal.Render(context, view, projection, light_direction);			//	「ポータル」の描画処理

    //DirectX::XMFLOAT4X4 world_view_projection;
    //DirectX::XMFLOAT4X4 world;

    //DirectX::XMMATRIX worldM = player.skinned_obj.GetWorldMatrix();
    ...
    //pFramework.player->render(context, world_view_projection, world, light_direction, material_color, false);

    player.Render(context, view, projection, light_direction);			//	「プレイヤー」の描画処理
    enemyManager.Render(context, view, projection, light_direction);		//	「敵管理」の描画処理
    shotManager.Render(context,view, projection, light_direction);		//	弾丸管理を描画
